...
if pagina == "Introdução":
    ...
    with img_col2:
        st.image("exoplanetas/capa.png", width=400, caption="Ilustração do sistema HD 209458")
    with img_col3:
        st.image("exoplanetas/superterra.jpg", width=350, caption="Ilustação do LHS 1140b, considerado uma Super Terra potencialmente habitável.")

    st.subheader("HD 209458b")
    st.image("exoplanetas/ilustracao.jpg", width=250, caption="Ilustração dramática do planeta HD 209458b queimando em sua orbita")
    ...
    with imge_col1:
        st.image("exoplanetas/radialhd.gif", width=400, caption="Velocidades para HD 209458 (anã G0). A linha sólida é um ajuste sinusoidal com um período de 3,5239 dias e uma semiamplitude de 81 m s-1, resultando em uma massa mínima M sen i = 0,62 MJup para o companheiro. O rms do ajuste sinusoidal é de 2,27 m s-1, menor do que os erros esperados de 3,7 m s-1.")
    with imge_col1:
        st.image("exoplanetas/transito hd.gif", width=400, caption="Observações fotométricas de HD 209458 da noite de 7 de novembro de 1999 UT mostrando a entrada do trânsito planetário. A profundidade de passagem medida é 0,017±0,002 mag ou 1,58%±0,18%. A barra de erro mostra o tempo de conjunção inferior e sua incerteza prevista a partir das velocidades radiais nesta Carta.")
    
    st.subheader("LHS 1140b")
    st.image("exoplanetas/esolhs.jpg", width=350, caption="Ilustração artística do sistema LHS 1140.")
    ...
    with lhs_col1:
        st.image("exoplanetas/radial_lhs.jpg", width= 400, caption= "Velocidade radial do LHS 1140 dos conjuntos de dados HARPS (vermelho) e ESPRESSO (aberto para ESPRESSOpre e preenchido para ESPRESSOpost em verde). A linha preta mostra o modelo de velocidade radial mediana da análise fotométrica e de velocidade radial conjunta. As regiões sombreadas em cinza correspondem aos intervalos de confiança de 68,7% (cinza escuro) e 95% (cinza claro) do modelo. O modelo GP mediano é mostrado como uma linha azul tracejada. Meio: conjunto de dados de velocidade radial após a remoção do modelo GP mediano. O modelo Kepleriano é mostrado como uma linha preta sólida. Abaixo: resíduos de velocidade radial do modelo completo.")
    with lhs_col2:
         st.image("exoplanetas/transitolhs.jpg", width= 400, caption= "Trânsito da curva de luz branca de LHS 1140 b com o WFC3 no HST. O modelo de transporte de transporte de melhor ajuste é mostrado em azul, com os resíduos do ajuste mostrados no painel inferior.")
